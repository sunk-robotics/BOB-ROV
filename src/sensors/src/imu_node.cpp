...
public:
  ...
  {
    this->declare_parameter("pub_rate_hz", kDefaultPubRateHz);
    ...
    this->declare_parameter("accel_variance", kDefaultAccelVar);
    this->declare_parameter("gyro_variance", kDefaultGyroVar);
    this->declare_parameter("mag_variance", kDefaultMagVar);
    ...
    imu_msg_enable = enable_orientation_ || enable_accel_ || enable_gyro_;
    ...
    if (!sensor_res) {
      auto msg =
          std::format("Failed to initilize IMU sensor! Error: {}", sensor_res.error().message());
      RCLCPP_FATAL(get_logger(), "%s", msg.c_str());
      throw std::runtime_error(msg);
    }
    ...
    auto timer_callback = [this]() -> void {
      if (imu_msg_enable) {
        ...
        if (enable_orientation_) {
          bno055_read_quaternion_wxyz(&orientation_vals_);
        }
        if (enable_accel_) {
          bno055_read_accel_xyz(&accel_vals_);
        }
        if (enable_gyro_) {
          bno055_read_gyro_xyz(&gyro_vals_);
        }
...
private:
  ...
  static constexpr int64_t kDefaultPubRateHz = 100;

...
pub async fn interface_handler() {
    ...
    #[cfg(feature = "imu")]
    let mut imu = init_imu(&i2c_refcell, &mut delay).await;

    #[cfg(feature = "aht20")]
    let mut aht20 = ahtx0(AtomicDevice::new(&i2c_refcell));
...
#[cfg(feature = "imu")]
async fn init_imu<'a>(
    i2c_refcell: &'a AtomicCell<rpi_pal::i2c::I2c>,
    delay: &mut hal::Delay
) -> bno055::Bno055<AtomicDevice<'a, rpi_pal::i2c::I2c>> {
    use std::fs;
    use bno055::{BNO055Calibration, BNO055OperationMode};

    let mut imu = bno055::Bno055::new(AtomicDevice::new(i2c_refcell));
    ...
    {            
        ...
        calibrate_imu(&mut imu, delay).await;
...
async fn calibrate_imu(
    imu: &mut bno055::Bno055<embedded_hal_bus::i2c::AtomicDevice<'_, rpi_pal::i2c::I2c>>, 
    ...
    loop {
        ...
        sleep(Duration::from_millis(config::imu::CALIB_READY_CHECK_INTERVAL)).await;
    }
...
fn write_imu_calib(
    ...
    if let Some(parent) = Path::new(config::imu::CALIB_CACHE_LOCATION).parent() 
        && !parent.exists() 
    {
        std::fs::create_dir_all(parent).unwrap_or_else(|err| panic!("Failed to create imu calib directory path! Error: {err}"));
        info!("Created cache directory structure!");
    }
